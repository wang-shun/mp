...
command="$javaexe -Dsun.misc.URLClassPath.disableJarChecking=true -Djava.security.egd=file:/dev/./urandom $JAVA_OPTS -jar $jarfile $RUN_ARGS $*"
...
do_start() {
  ...
  if [[ -n "$run_user" ]]; then
    ...
    if [ $USE_START_STOP_DAEMON = true ] && type start-stop-daemon > /dev/null 2>&1; then
      arguments=(-Dsun.misc.URLClassPath.disableJarChecking=true -Djava.security.egd=file:/dev/./urandom $JAVA_OPTS -jar $jarfile $RUN_ARGS "$@")
