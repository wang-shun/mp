			...
			var AddSnippetView = CommunicationBaseView
					.extend({
						...
						_setContentHTML : function (){
							...
							commonDialog.addBtn("publish","发布",function(){
								self._publishForm(commonDialog);
							});
						...
						_dialogBtnInit:function(tab){
							...
							if (tab == "base-info") {
								...
								this.dialog.addBtn("prev","上一步",function(){									
									if($("li.SID-priv-li.rank-item").not("li.SID-priv-li.rank-item.SID-user-li").length!=0){
		                ...
		                _fillData : function(data){
		                	...
		                	if(data.meeting.hasGroup == 1)
		                		this.$el.find(".sel_yes>input[type='radio']").attr('checked','checked');
		                	if(data.meeting.signType == 1)
		                		this.$el.find(".signType_close>input[type='radio']").attr('checked','checked');
		                	//填充会议议程
		                	...
		                	for(var i=0;i<mdList.length;i++){
		                		if(mdList[i].type == 'inner' && mdList[i].entityType == 'dept'){
		                			...
		                			var name = mdList[i].entityName;
		                			var cutname = name;
		                			if(cutname.length>4){
										cutname = cutname.substr(0,4);
									}
		                			var text = "<span class='name' data-name='"+name+"'>"+cutname+"</span><span class='num'>"+mdList[i].entityId+"</span>";
		                			userBox.append("<li class='SID-priv-li rank-item' data-type='user' data-depUuid='"+mdList[i].entityId+"'><i class='fhicon-close3 SID-del-li'></i>"+text+"</li>");
						...
						_onClickOuterOk:function(){
							...
							if (name == "" || phone == "") {
								...
								if (mobile.test(phone)) {
				            		...
				            		userBox.append("<li class='SID-priv-li rank-item' data-type='user' data-depUuid='"+phone+"'><i class='fhicon-close3 SID-del-li'></i><span class='name' data-name='"+name+"'>"+cutname+"</span><span class='num'>"+phone+"</span></li>");
				        ...
				        _saveForm:function(commonDialog){
				        	var method = "mapps.meeting.meeting.service.add";
				        	this._sumbitForm(commonDialog,method,"保存会议");
				        },
						...
						_sumbitForm : function(commonDialog,method,title){
						    ...
						    if(this.validateResult()){
				                ...
				                meetingInfo.signType =$('input[name="signType"]:checked').val();
				                ...
				                for(var i=0,li;li=outerLi[i++];){
					                ...
					                inInfo2.entityName = $(li).find("span[class='name']").attr("data-name");
				                ...
				                if (data.serviceParticipantsList.length > 0 || data.signinSequList.length > 0){
				                	data.signinFlag=1;
				                	if (data.signinSequList.length < 1) {
				                		this.$el.find(".SID-tab-btn[data-name='meeting-sign']").click();
					                	fh.alert("请添加签到",false,function(){ });
					                	return;
				                	}
				                }
				                // 会议备注
								...
								$.ajax({
									type : "POST",
									url:encodeURI(servicePath+"?"+ropParam+ "&method="+method+"&meetingJson="+param),
