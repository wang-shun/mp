...
public class ThirdPartAccessMosService
{
    ...
    public JoinImGroupResponse joinImGroup(JoinImGroupRequest request) throws JoinImGroupException
    {
        ...
        try
        {
            LOGGER.debug("群组加入成员接口调用,serviceUrl:{},appKey:{},appSecret:{},ecid:{},username:{},GroupId:{},Members:{}",
    ...
    @ServiceMethod(method = "mapps.thirdpart.mobileark.deletemember", group = "thirdpart", groupTitle = "文件服务", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
    public JoinImGroupResponse deleteMembersFromImGroup(JoinImGroupRequest request) throws DeleteMembersFromImGroupException
    {
        JoinImGroupResponse response = new JoinImGroupResponse();
        try
        {
            LOGGER.debug("群组删除成员接口调用,serviceUrl:{},appKey:{},appSecret:{},ecid:{},username:{},GroupId:{},Members:{}",
                    webRoot, appKey, appSecret, SessionContext.getEcId(), SessionContext.getUserId(),
                    request.getGroupId(), request.getMembers());
            Map<String, Object> reqMap = new HashMap<String, Object>();
            reqMap.put("ecid", SessionContext.getEcId());
            ...
            reqMap.put("members", request.getMembers());
            RopClient client = new RopClient(webRoot, appKey, appSecret, "json");
            String result = client.requestForString("mos.thirdpart.im.group.deletemember", "1.0", reqMap);
            LOGGER.debug("======群组删除成员响应结果======" + result);
            JSONObject data = JSONObject.fromObject(result);
            if (data.containsKey("code") && "1".equals(data.get("code")))
            {
                response.success();
        ...
        {
            LOGGER.error("群组删除成员失败：{}", e);
            ErrorCode.fail(response, ErrorCode.CODE_100001);
            throw new DeleteMembersFromImGroupException();
        }
        return response;
    }
