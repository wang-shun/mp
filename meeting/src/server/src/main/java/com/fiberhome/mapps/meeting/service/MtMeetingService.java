...
public class MtMeetingService
{
     ...
     * @return
     */
    @ServiceMethod(method = "mapps.meeting.meeting.service.add", group = "meeting", groupTitle = "API", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
    @Transactional
    public AddMeetingResponse addMeetingFromService(AddMeetingRequest req)
    {
        LOGGER.info("新增会议接口(mapps.meeting.meeting.service.add)入口,请求参数==" + req.getMeetingJson());
        ...
        try
        {
            ...
            mtMeetingInfo.setStatus(STAUTS_10);
            // mtMeetingInfo.setReleaseTime(new Date());
            if (StringUtil.isNotEmpty(addInfo.getMeeting().getId()))
            {
                // 会议信息更新
                meetingId = mtMeetingInfo.getId();
                opFlag = false;
                // 删除会议主题信息外的其他信息
    ...
    public void insertMeetingInfo(String meetingId, AddMeetingRequest addInfo) throws Exception
    {
        ...
        if (FLAG_OPEN.equals(addInfo.getSigninFlag()))
        {
            ...
            for (ClientMtSigninSequInfo info : signinSequList)
            {
                ...
                info.setMeetingId(meetingId);
                info.setSequ(sequ);
                info.setQrcode(getQrcode("!sign20!" + meetingId + "::" + sequ));
    ...
    @ServiceMethod(method = "mapps.meeting.meeting.service.publish", group = "meeting", groupTitle = "API", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
    @Transactional
    public AddMeetingResponse addMeetingFromClientV20(AddMeetingRequest req)
    {
        LOGGER.info("新增会议接口(mapps.meeting.meeting.service.publish)入口,请求参数==" + req.getMeetingJson());
        AddMeetingResponse res = new AddMeetingResponse();
        try
        {
            AddMeetingRequest addInfo = (AddMeetingRequest) JsonUtil.jsonToObject(req.getMeetingJson(),
    ...
    public boolean joinImGroup(String groupId, Map<String, String> map) throws Exception
    {
    ...
    public QueryMeetingResponse getMeetingForWeb(QueryMeetingServerRequest req)
    {
        ...
        try
        {
            ...
            if (StringUtil.isNotEmpty(req.getMeetingHolder()))
            {
                ...
                // thirdPartAccessService.getUsers(request);
    ...
    public String queryQrcode(String meetingId, int num)
    {
        Map<String, Object> map = new HashMap<String, Object>();
        ...
        if (num != -1)
        {
            reStr = fileService.getWebRoot() + getQrcode(mt.getId() + "::" + num);
        }
        else
        {
            if ("-1".equals(mt.getQrcode()))
            {
                ...
                if (!"-1".equals(reStr))
                {
                    reStr = fileService.getWebRoot() + reStr;
                }
            }
            else
            {
                reStr = fileService.getWebRoot() + mt.getQrcode();
            }
        }
        return reStr;
    }
    ...
    public QueryMeetingDetailResponse getMeetingDetail(QueryMeetingDetailRequest req)
    {
        ...
        try
        {
            ...
            if (req.isRefreshFlag())
            {
                ...
                // 根据meetingId和personId查询参会者id
                String reCode = queryQrcode(req.getMeetingId(), -1);
            ...
            // 签到信息
            List<ClientMtSigninSequInfo> list = mtSigninSequMapper.getByMeetingId(map);
            for (ClientMtSigninSequInfo sequ : list)
            {
    ...
    public QueryMeetingDetailResponse getOuterDetail(QueryOuterDetailRequest req)
    {
        ...
        try
        {
            ...
            res.setQrcode("-1".equals(code) ? code : fileService.getWebRoot() + code);
            // 议程信息
            List<String> days = mtAgendaMapper.getDayByMeetingId(map);
            List<MtAgenda> agendaList = mtAgendaMapper.getByMeetingId(map);
            List<ClientAgendaInfo> caInfoList = new ArrayList<ClientAgendaInfo>();
    ...
    public CancelMeetResponse cancelMeeting(CancelMeetRequset req)
    {
        ...
        try
        {
            ...
            reqMap.put("databaseType", databaseType);
                    ...
                    || meeting.getStatus().equals(MtMeetingService.STAUTS_30))
            {
                ...
                // 获取参会人员登录名
                // 推送给参会人员
                String title = meeting.getMeetingName() + "会议已被取消";
                String content = "原定于" + meeting.getBeginTimeStr() + "-" + meeting.getEndTimeStr() + "的"
                        + meeting.getMeetingName() + "会议已被预订者取消";
                sendMsg(req.getMeetingId(), title, content);
    ...
    public void sendMsg(String meetingId, String title, String content)
    {
        ...
        thirdPartAccessService.pushReceiveEvent(SessionContext.getEcId(), StringUtils.join(loginIds, ","), title,
                content, meetingId);
    ...
    public SignRecordResponse querySequ(QueryMeetingDetailRequest req)
    {
        ...
        try
        {
            ...
            Map<String, Object> map = new HashMap<String, Object>();
            map.put("meetingId", req.getMeetingId());
            List<ClientMtSigninSequInfo> msq = mtSigninSequMapper.getByMeetingId(map);
    ...
    public SignRecordResponse querySign(QueryMeetingDetailRequest req)
    {
        ...
        try
        {
            ...
            map.put("meetingId", req.getMeetingId());
            List<ClientMtSigninSequInfo> msq = mtSigninSequMapper.getByMeetingId(map);
            for (ClientMtSigninSequInfo mssInfo : msq)
            {
                colSql.append(",max(stime" + mssInfo.getSequ() + ") as \"sequ" + mssInfo.getSequ() + "\" ");
                caseSql.append(",case when ss.sequ = '" + mssInfo.getSequ()
                        + "' then "
                        + ("db/mysql".equals(databaseType)?"date_format(sign_time,'%Y-%m-%d %H:%i:%s')":"to_char(sign_time,'yyyy-mm-dd hh24:mi:ss')")
                        + " else null end stime" + mssInfo.getSequ()
                        + " ");
            }
            ...
            ArrayList<SignDetail> srrList = new ArrayList<SignDetail>();
            for (int i = 0; i < sdList.size(); i++)
            {
                SignDetail detail = new SignDetail();
                HashMap<String, Object> temp = sdList.get(i);
                detail.setPersonType((String) temp.get("person_type"));
                detail.setPersonName((String) temp.get("person_name"));
                detail.setPersonId((String) temp.get("person_id"));
                Map<String, String> signInfo = new HashMap<String, String>();
                for (int k = 1; k < msq.size() + 1; k++)
                {
                    signInfo.put("sequ" + k, (String) temp.get("sequ" + k));
                }
                detail.setSignInfo(signInfo);
                srrList.add(detail);
            }
            // JSONArray json = JSONArray.fromObject(sdList);
            // @SuppressWarnings("unchecked")
            // List<SignDetail> srrList = (List<SignDetail>)
            // JsonUtil.jsonToObject(json.toString(), SignDetail.class);
            // 调用第三方接口查询用户信息
            ...
            for (SignDetail sd : srrList)
            {
                ...
                {
                    LOGGER.debug("personId:{}", sd.getPersonId());
    ...
    public void scheduleChangeStatus()
    {
        ...
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("now", now);
        mtMeetingMapper.updateStatusSchedule(map);
        LOGGER.debug("刷新会议状态定时任务执行完成,执行时间:{}", now);
    }
    ...
    public void scheduleMeetingCall() throws Exception
    {
        ...
        for (MtMeeting info : meetingList)
        {
            ...
            {
                pushMeetingCall(info, mpInfo.getPersonId(), username);
            }
    ...
    @Async
    public void pushMeetingCall(MtMeeting mtMeetingInfo, String loginId, String username)
    {
        String title = "您参加的会议还有";
        if ("1".equals(mtMeetingInfo.getNoticeType()))
        {
            title += mtMeetingInfo.getNoticeSet() + "分钟";
        }
        else if ("2".equals(mtMeetingInfo.getNoticeType()))
        {
            title += mtMeetingInfo.getNoticeSet() + "小时";
        }
        title += "开始。";
        ...
        sb.append("\n召开人：");
        sb.append(username);
        thirdPartAccessService.pushReceiveEvent(mtMeetingInfo.getEcid(), loginId, title, sb.toString(),
                mtMeetingInfo.getId());
    }
    ...
    @Async
    @Transactional
    public void pushMsg(MtMeeting mtMeetingInfo, List<String> idList, String username)
    {
        String title = username + "邀请您参加会议。";
        StringBuilder sb = new StringBuilder();
        sb.append("会议名称：");
        sb.append(mtMeetingInfo.getName());
        sb.append("\n会议时间：");
        ...
        sb.append("\n召开人：");
        sb.append(username);
        if (thirdPartAccessService.pushReceiveEvent(mtMeetingInfo.getEcid(), StringUtils.join(idList, ","), title,
    ...
    public QueryMeetingDetailResponseV11 getMeetingDetailV11(QueryMeetingDetailRequest req)
    {
        ...
        try
        {
            ...
            for (ClientMtSigninSequInfo sequ : list)
            {
                ...
                sequ.setSequStr(ConvertUtil.numberToChinese(num));
                String reStr = "-1";
                if ("-1".equals(sequ.getQrcode()))
                {
                    reStr = getQrcode("!sign20!" + req.getMeetingId() + "::" + num);
                    if (!"-1".equals(reStr))
                    {
                        reStr = fileService.getWebRoot() + reStr;
                    }
                ...
                {
                    reStr = fileService.getWebRoot() + sequ.getQrcode();
                }
                sequ.setQrcode(reStr);

                // 根据meetingId和personId查询参会者id
                String reCode = queryQrcode(req.getMeetingId(), num);
                if (ErrorCode.CODE_100001.equals(reCode))
                {
                    ErrorCode.fail(res, ErrorCode.CODE_100001);
                    LOGGER.info("未查询到相关参会信息,meetingid:{},userId:{}", req.getMeetingId(), SessionContext.getUserId());
                ...
                qrCodeList.add(reCode);
            }
            res.setSigninSequList(list);
            res.setQrcodeList(qrCodeList);
            // 查询初始参会人员选择信息
            Map<String, Object> mdMap = new HashMap<String, Object>();
            mdMap.put("meetingId", req.getMeetingId());
            res.setOriginParticipantsList(mdParticipantsMapper.getByMeetingId(mdMap));
            // 查询参会人信息
            ...
            if (list != null && list.size() > 0)
            {
                ...
                // map.put("allFlag", "-1");
                // // 签到信息
                // List<MtSigninRecord> signinPersonList =
                // mtSigninRecordMapper.getBySequId(map);
     ...
     * 正向签到 通过扫描会议的二维码自行签到
     */
    @ServiceMethod(method = "mapps.meeting.meeting.signin", group = "room", groupTitle = "API", version = "2.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
    public SignInResponse signInV20(SignInRequest req)
    {
        LOGGER.info("扫描会议二维码接口(mapps.meeting.meeting.signin)入口,请求参数==" + LogUtil.getObjectInfo(req));
        SignInResponse res = new SignInResponse();
        try
        {
            LOGGER.info("签到入口" + LogUtil.getObjectInfo(req));
            Map<String, Object> map = new HashMap<String, Object>();
            String signStr = req.getParticipantId();
            String sequId = req.getSequId();
            String meetingId = "";
            String userId = "";
            String userName = "";
            String serId = "";
            long sequ = 0;
            // 签到2.0新增正向签到
            if (signStr.contains("!sign20!"))
            {
                String[] arr = signStr.split("!sign20!");
                if (arr == null || arr.length != 2)
                {
                    LOGGER.debug("签到异常：无效参数,{}", signStr);
                    ErrorCode.fail(res, ErrorCode.CODE_100001);
                    return res;
                }
                String[] arrMsg = arr[1].split("::");
                meetingId = arrMsg[0];
                sequ = Long.parseLong(arrMsg[1]);
                MtSigninSequ record = new MtSigninSequ();
                record.setMeetingId(meetingId);
                record.setSequ(sequ);
                sequId = mtSigninSequMapper.selectOne(record).getId();
                MtMeeting info = mtMeetingMapper.selectByPrimaryKey(meetingId);
                if (info == null || !"0".equals(info.getSignType()))
                {
                    LOGGER.debug("签到异常：无效会议,{}", meetingId);
                    ErrorCode.fail(res, ErrorCode.CODE_100001);
                    return res;
                }
            ...
            else
            {
                ...
                String signPersonId = arr[0];
                map.put("id", signPersonId);
                MtParticipants mt = mtParticipantsMapper.getParById(map);
                if (mt == null)
                {
                    LOGGER.debug("签到异常：无效参数,{}", signStr);
                ...
                meetingId = mt.getMeetingId();
                userId = mt.getPersonId();
                serId = SessionContext.getUserId();
                userName = mt.getPersonName();
                sequ = Long.parseLong(arr[1]);
                MtSigninSequ record = new MtSigninSequ();
                record.setMeetingId(meetingId);
                record.setSequ(sequ);
                sequId = mtSigninSequMapper.selectOne(record).getId();
            ...
            // 签到
            map.put("meetingId", meetingId);
            map.put("sequId", sequId);// 签到序列
            map.put("personId", userId);// 参会人员
            map.put("serId", serId);// 服务人员
            MtSigninRecord mr = mtSigninRecordMapper.getUpdateData(map);
            if (mr != null)
            {
                if (mr.getSigned().equals("Y") || mr.getSignTime() != null)
