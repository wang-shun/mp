define([
    ...
    var view = Backbone.View.extend({
        ...
        loadGroup:function(){
            ...
            var ropParam = app.ropMethod.getDepartments;
            $.ajax({
            	url : url,
				data : ropParam,
        ...
        loadPeople:function(e){
            ...
            self.loadPanel.show();
            var url = window.serviceUrl;
            var ropParam = app.ropMethod.getUsers+"&depUuid="+id+"&depScope=0";
            $.ajax({
            	url : url,
				data : ropParam,
                success:function(data){
...
//                        }
        ...
        _onSubmit:function(e){
            ...
            for(var n in selected){
                ...
                if(selected[n].loginid || selected[n].type=="1"){
                    type="1";
                }
            ...
            //赋值
            /*$("[name='NID-MeetAdd-PeopleName']").val(name);
            $("[name='NID-MeetAdd-People']").val(opts);*/
            $("[name='"+app.peopleNameInput+"']").val(name);
            $("[name='"+app.peopleInput+"']").val(opts);
            //返回上一页
            this._reset();
            history.go(-1);
