...
/**
 * This parses a span tree into dependency links used by Web UI. Ex.
 * http://zipkin/dependency
 *
 * <p>
 * This implementation traverses the tree, and only creates links between
 * {@link DependencyLinkSpan.Kind#SERVER server} spans. One exception is at the
 * bottom of the trace tree. {@link DependencyLinkSpan.Kind#CLIENT client} spans
 * that record their {@link DependencyLinkSpan#peerService peer} are included,
 * as this accounts for uninstrumented services.
 */
public final class DependencyLinker {
	...
	/**
	 * @param spans
	 *            spans where all spans have the same trace id
	 */
	...
	public DependencyLinker putTrace(Iterator<DependencyLinkSpan> spans) {
		...
		for (Iterator<Node<DependencyLinkSpan>> i = tree.traverse(); i.hasNext();) {
			...
			// ancestor
			// Look up the stack until we see a service name, and assume that's
			// the client
	...
	public List<DependencyLink> link() {
		// links are merged by mapping to parent/child and summing corresponding
		// links
	...
	String getChild(JSONObject source, String id) {
		DependencyLinkSpan childSpan = (DependencyLinkSpan) source.get(id);
		if (childSpan != null) {
			String child = childSpan.service;
			if ("mapps-gateway".equals(child)) {
				child = getChild(source, Util.toLowerHex(childSpan.id));
			}
			return child;
		} else {
			return null;
	...
	/**
	 * 处理trace链,剔除mapps-gateway的中转 modified by 张文剑,fiberhome
	 * 
	 * @param spans
	 *            spans where all spans have the same trace id
	 */
	public DependencyLinker putTraceExceptGateway(Iterator<DependencyLinkSpan> spans) {
		if (!spans.hasNext())
			return this;

		// 处理trace链,剔除mapps-gateway的中转 modified by 张文剑,fiberhome
		JSONObject spanSourceById = new JSONObject();
		JSONObject spanSourceByParentId = new JSONObject();

		Node.TreeBuilder<DependencyLinkSpan> builder = new Node.TreeBuilder<>();
		while (spans.hasNext()) {
			DependencyLinkSpan next = spans.next();
			builder.addNode(next.parentId, next.id, next);
			spanSourceById.put(Util.toLowerHex(next.id), next);
			if (next.parentId != null)
				spanSourceByParentId.put(Util.toLowerHex(next.parentId), next);
		}
		Node<DependencyLinkSpan> tree = builder.build();
		...
		for (Iterator<Node<DependencyLinkSpan>> i = tree.traverse(); i.hasNext();) {
			...
			case SERVER:
				...
				// 处理trace链,剔除mapps-gateway的中转 modified by 张文剑,fiberhome
				String id = Util.toLowerHex(current.value().id);
				TraceId traceIdorgin = current.value().traceId;
				String traceId = Util.toLowerHex(traceIdorgin.hi, traceIdorgin.lo);
				Long parentIdorgin = current.value().parentId;
				if (parentIdorgin != null) {
					String parentId = Util.toLowerHex(parentIdorgin);
					if ("mapps-gateway".equals(parent) && !traceId.equals(parentId)) {
						DependencyLinkSpan parentSpan = (DependencyLinkSpan) spanSourceById.get(parentId);
						if (parentSpan != null) {
							parent = parentSpan.peerService;
						} else {
							parent = null;
						}
					}
				...
				if ("mapps-gateway".equals(child)) {
					child = getChild(spanSourceByParentId, id);
				}
