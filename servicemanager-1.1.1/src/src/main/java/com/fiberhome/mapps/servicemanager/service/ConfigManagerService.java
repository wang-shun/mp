...
import com.fiberhome.mapps.servicemanager.request.ServiceBindRequest;
...
public class ConfigManagerService {
	...
	public BaseResponse serviceBind(ServiceBindRequest request) {
		...
		// 写入配置
		try {
			FileUtils.writeByteArrayToFile(configFile, yaml.dump(config).getBytes("UTF-8"));
	...
	private RestartResult restart(String application) throws IOException {
		...
		for (InstanceInfo instance : instances) {
			...
			if ((InstanceStatus.UP.equals(instance.getStatus()) || InstanceStatus.DOWN.equals(instance.getStatus())) && !StringUtils.isEmpty(statusUrl)) {
				...
				try {
					...
					HttpResponse httpres = HttpClients.createDefault().execute(request);
					String response = restTemplate.postForObject(restartUrl, null, String.class);
			...
			for (ResourceAssignInfo rai : raiList) {
				if (rai.getResId().equals("database")) {
					...
					if (dbAssign != null) {
						...
						rai.setConfig(getDatabaseResourceObjectInfo(rsdb));
					}
					...
					if (dbAssign != null) {
						...
						rai.setConfig(getRedisResourceObjectInfo(rsrds));
					}
				} else {
	...
	class SortByAppId implements Comparator {
		public int compare(Object o1, Object o2) {
			AppInfo s1 = (AppInfo) o1;
			AppInfo s2 = (AppInfo) o2;
			return s1.getAppId().compareTo(s2.getAppId());
		}
	}

	class SortByAppIdDesc implements Comparator {
		public int compare(Object o1, Object o2) {
			AppInfo s1 = (AppInfo) o1;
			AppInfo s2 = (AppInfo) o2;
	...
	public AppConfigResponse appConfigLoad(AppIdRequest req) {
		...
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("appId", req.getAppId());
			List<RsKeyValue> cfglist = appConfigMapper.getConfigInfo(map);
	...
	public AppSaveFormResponse appSaveform(AppSaveFormRequest req) {
		...
		try {
			...
			//List<ServiceInstance> siList = discovery.getInstances(req.getAppId());
			Applications apps = eurekaClient.getApplications();
	...
	public List<RsKeyValue> getDatabaseResourceObjectInfo(Object obj) {
		try {
			List<RsKeyValue> rsJsonMap = new ArrayList<RsKeyValue>();
			// 得到类对象
			...
			rsJson.setValue(CodecUtil.aesDecrypt(db.getPassword(), DBPasswordEncrypt.encryptKey));
			rsJsonMap.add(rsJson);
			return rsJsonMap;
		} catch (Exception e) {
	...
	public List<RsKeyValue> getRedisResourceObjectInfo(Object obj) {
		try {
			List<RsKeyValue> rsJsonMap = new ArrayList<RsKeyValue>();
			// 得到类对象
			...
			rsJson = new RsKeyValue();
			rsJson.setKey("password");
			rsJson.setValue(CodecUtil.aesDecrypt(rds.getPassword(), DBPasswordEncrypt.encryptKey));
			rsJsonMap.add(rsJson);
			return rsJsonMap;
		} catch (Exception e) {
