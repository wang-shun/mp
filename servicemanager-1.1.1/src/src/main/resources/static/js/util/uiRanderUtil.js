define(["spin", "underscore", "jqueryUi", "jqueryUiSlide", "jqueryUiTimepicker"], 
	function(Spinner, _){
		...
		UiRanderUtil.randerJQueryUI_DateTimeRange = function(view, fromSid, toSid) {
			var self = this;
			var myDate = new Date();
			var mi = myDate.getMinutes();
			myDate.setMinutes(myDate.getMinutes()+(5-mi%5));
			var from = $(fromSid).datetimepicker({stepMinute: 5,minDate: myDate});
			var to = $(toSid).datetimepicker({stepMinute: 5,minDate: myDate});
		};
		UiRanderUtil.randerDateTime = function(obj) {
			var from = obj.datetimepicker({stepMinute: 5});
		};
		return UiRanderUtil;
});