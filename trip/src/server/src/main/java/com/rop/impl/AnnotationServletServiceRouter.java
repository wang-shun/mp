...
import com.rop.marshaller.GzipServletOutputStream;
...
import com.rop.utils.CalcObjectSizeOutputStream;
...
public class AnnotationServletServiceRouter implements ServiceRouter
{
    ...
    // 自动启用压缩流的响应流大小，大于该数值自动启用json的gzip压缩, 单位为byte，-1，不启用，0始终压缩
    private int                          doGzipStreamSize        = 100000;
    // 会话管理器
    ...
    public void service(Object request, Object response)
    {
        ...
        {// 超过最大的服务平台的最大资源限制，无法提供服务
            ...
            writeResponse(ropResponse, servletResponse, ropRequestContext);
    ...
    private class ServiceRunnable implements Runnable
    {
        ...
        public void run()
        {
            ...
            try
            {
                ...
                // 输出响应
                writeResponse(ropRequestContext.getRopResponse(), servletResponse, ropRequestContext);
            } catch (Throwable e)
            {
    ...
    private void writeResponse(Object ropResponse, HttpServletResponse httpServletResponse, RopRequestContext ropRequestContext)
    {
        MessageFormat messageFormat = ropRequestContext.getMessageFormat();
        try
        {
            ...
            if (messageFormat == MessageFormat.xml)
            {
            	...
            	if (doGzipStreamSize == 0) {
            		dogzip = true;
            	} else if (doGzipStreamSize == -1){
            		dogzip = false;
            	} else {
            		CalcObjectSizeOutputStream calcStream = new CalcObjectSizeOutputStream();
            		xmlMarshallerRop.marshaller(ropResponse, calcStream);
                	long responseSize = calcStream.getSize();
                	calcStream.close();
                	dogzip = (responseSize > doGzipStreamSize);
            	}
            	...
            	if(dogzip){
                    ...
                    // 设置字符集  
               	    ...
               	    GzipServletOutputStream gzipStream = new GzipServletOutputStream(httpServletResponse.getOutputStream());
               	    try {
               	    	xmlMarshallerRop.marshaller(ropResponse, gzipStream);
               	    }
               	    finally {
               	    	gzipStream.close();
               	    }
            ...
            } else if (messageFormat == MessageFormat.bin)
            {
                if (ropResponse instanceof FileResponse)
                {
                    ((FileResponse) ropResponse).marshaller(httpServletResponse);
                } else if (ropResponse instanceof ErrorResponse) {
                    logger.warn(MessageMarshallerUtils.getMessage(ropResponse, MessageFormat.xml));
                    httpServletResponse.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                } else
                {
                    httpServletResponse.setContentType(APPLICATION_BIN);
                    new ObjectOutputStream(httpServletResponse.getOutputStream()).writeObject(ropResponse);
                }
            } else if (messageFormat == MessageFormat.jsonp)
            {
                String jsoncallback = ropRequestContext.getParamValue("jsoncallback");
                httpServletResponse.setContentType(APPLICATION_TEXT);
                httpServletResponse.getOutputStream().print(jsoncallback + "(");
                jsonMarshallerRop.marshaller(ropResponse, httpServletResponse.getOutputStream());
                httpServletResponse.getOutputStream().print(")");
                
            }
            /*else if(messageFormat == MessageFormat.json_gzip){//json_gzip   2013-07-26 增加大数据压缩格式
...
//            	 response.setContentLength(output.length);  
            	 jsonGZIPMarshallerRop.marshaller(ropResponse, httpServletResponse.getOutputStream());
            	
            } else if(messageFormat == MessageFormat.xml_gzip){//xml_gzip   2013-07-26 增加大数据压缩格式
	             ...
	             // 设定输出流中内容长度  
//	             response.setContentLength(output.length);  
	           	 xmlGZIPMarshallerRop.marshaller(ropResponse, httpServletResponse.getOutputStream());
            ...
            else
            {
            	...
            	if (doGzipStreamSize == 0) {
            		...
            		CalcObjectSizeOutputStream calcStream = new CalcObjectSizeOutputStream();
                	jsonMarshallerRop.marshaller(ropResponse, calcStream);
                	long responseSize = calcStream.getSize();
                	calcStream.close();
                	dogzip = (responseSize > doGzipStreamSize);
            	}
            	...
            	if(dogzip){
                    ...
                    // 设置字符集  
               	    ...
               	    GzipServletOutputStream gzipStream = new GzipServletOutputStream(httpServletResponse.getOutputStream());
               	    try {
               	    	jsonMarshallerRop.marshaller(ropResponse, gzipStream);
               	    }
               	    finally {
               	    	gzipStream.close();
               	    }
    ...
    public void setDoGzipStreamSize(int doGzipStreamSize) {
		this.doGzipStreamSize = doGzipStreamSize;
	}
}