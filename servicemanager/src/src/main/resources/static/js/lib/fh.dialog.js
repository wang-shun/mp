...
fh.showInfo = function (p,handler,cancelHandler) {
	var obj = {id:(p.id ? p.id : "fhdialog"),xButton:(!p.xButton ? p.xButton : true),parent:(p.parent ? p.parent : ""), title:(p.tit ? p.tit : "\u53cb\u60c5\u63d0\u793a\uff1a"), width:(p.w ? p.w : 300), height:(p.h ? p.h : 160), cover:(p.c ? p.c : true), rang:(p.r ? p.r : true), ischild:(p.ischild ? p.ischild : false), resize:(p.re ? p.re : false), html:("<div id='info_con'><table width='100%' height='100%' border='0' cellspacing='0' cellpadding='0'><tr><td align='center' class='alert-icon' width='75px'><div class='" + (p.type ? p.type : "alert") + "_info'>&nbsp;</div></td><td class='alert-word'><div class='info_txt'>" + p.message + "</div></td></tr></table></div>"), dgOnLoad:function () {
	    if (p.type == "confirm") {
	       if(cancelHandler){
	...
	if(dlg){
		...
		$('#lhgdlg_'+(p.id ? p.id : "fhdialog"), document).focus();
	}
};
 ...
 *            参数
 */
fh.commonOpenDialog = function(id, title, width, height, html, handle, param, resize,
    parentId) {
  var args = {};
  if (handle != null) {
    args.handle = handle;
  ...
  if (parentId) {
    obj.parent = parentId;
    var commonDialog = new obj.parent.curWin.$.dialog(obj);
  } else {
...
fh = (window.parent.fh);
fh.servicemanagerOpenDialog =  function (a, b, c, d, e, f, g, h, i) {
	var j = {};
	null != f && (j.handle = f),
	null != g && (j.param = g);
	var k = {
	...
	if (i) {
		k.parent = i;
		var l = new k.parent.curWin.$.dialog(k);
	} else {
		var l = new window.parent.$.dialog(k);
	}
	return l.ShowDialog(),
	l
};
fh.servicemanagerAlert = function (info,ischild,handler,parentDlg,dlgparam) {
	var p = {id:'fhdialog-alert',message:info,type:'alert'};
	if(ischild){
	    p = {id:'fhdialog-alert',message:info,type:'alert',ischild:true,parent:parentDlg};
	}
	...
	if(handler){
	   fh.servicemanagerShowInfo(p,handler);
	}else{
	   fh.servicemanagerShowInfo(p);
	}
};
fh.servicemanagerShowInfo = function (p,handler,cancelHandler) {
	var obj = {id:(p.id ? p.id : "fhdialog"),xButton:(!p.xButton ? p.xButton : true),parent:(p.parent ? p.parent : ""), title:(p.tit ? p.tit : "\u53cb\u60c5\u63d0\u793a\uff1a"), width:(p.w ? p.w : 300), height:(p.h ? p.h : 160), cover:(p.c ? p.c : true), rang:(p.r ? p.r : true), ischild:(p.ischild ? p.ischild : false), resize:(p.re ? p.re : false), html:("<div id='info_con'><table width='100%' height='100%' border='0' cellspacing='0' cellpadding='0'><tr><td align='center' class='alert-icon' width='75px'><div class='" + (p.type ? p.type : "alert") + "_info'>&nbsp;</div></td><td class='alert-word'><div class='info_txt'>" + p.message + "</div></td></tr></table></div>"), dgOnLoad:function () {
	    if (p.type == "confirm") {
	       if(cancelHandler){
	           dlg.addBtn("cancel","取消",function(){
