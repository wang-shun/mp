...
//										}
		                	...
		                	var measurement = self.$el.find('.SID-measurement').val().split("-")[0];
							if(measurement != ""){
								$.ajax({
									type:"POST",
					                	...
					                	if(self.data.ruleDetail != null){
					                		if(self.data.ruleDetail.alertRule.groupBy != ""){
						                		...
						                		for(var k=0;k<groupbyList.length;k++){
						                			self.$el.find("#rule"+groupbyList[k]+"groupby").attr("checked","checked");
						                		}
					                		}
					                		if(self.data.ruleDetail.alertRule.whEre != ""){
					                			var whereList = self.data.ruleDetail.alertRule.whEre.split(" or ");
						                		for(var k=0;k<whereList.length;k++){
						                			var wherekey = whereList[k].split("=")[0].replaceAll("\"","");
						                			var wherevalue = whereList[k].split("=")[1].replaceAll("'","");
						                			self.$el.find("#rule"+wherekey+"tag"+wherevalue).attr("checked","checked");
						                		}
					                		}
					                		self._clickFilter();
										}
						...
						_clickFilter:function(e){
							...
							for(var i=0;i<groupbyList.length;i++){
								if($(groupbyList[i]).attr("checked") == "checked"){
									...
									//定义标签tag模板
									var LabelTagTemplate = '<a class="meet-add SID-insertword" style="padding: 0 10px;display:inline-block" ttile="tag标签">{{ index .Tags "'+$(groupbyList[i]).attr("data-tag")+'" }}</a>';
...
//                        	}
						...
						_sumbitForm : function(method,title,closeMethod){
						    ...
						    if(this.validateResult() && extraValidate.checkFlag){
	                        	var measurement = _self.$el.find(".SID-measurement").val().split("-")[0];
