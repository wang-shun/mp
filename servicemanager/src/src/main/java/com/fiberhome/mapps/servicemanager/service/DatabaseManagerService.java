...
public class DatabaseManagerService {
	...
	public DatabaseDetailResponse getDetail(DatabaseDetailRequest req) {
		...
		try {
			...
			databaseDetail
					.setPassword(CodecUtil.aesDecrypt(databaseDetail.getPassword(), DBPasswordEncrypt.encryptKey));
	...
	public DatabaseDetailResponse addDetail(DatabaseSaveRequest req) {
		...
		try {
			...
			if ("postgresql".equals(database.getDbType()) || "mysql".equals(database.getDbType())) {
	...
	public DatabaseDetailResponse createDb(DatabaseCreateRequest req) {
		LOGGER.debug("===创建数据库服务接口(mapps.servicemanager.database.create)入口,请求参数==" + req.getDatabaseJson() + "==="
				+ req.getSidordbname() + "===" + req.getAdminuser() + "===" + req.getAdminpass());
		DatabaseDetailResponse response = new DatabaseDetailResponse();
		try {
			...
			// 数据库连接
			String conUrl = "";
			if ("postgresql".equals(database.getDbType())) {
				database.setDbName(req.getSidordbname());
				driverClass = "org.postgresql.Driver";
				conUrl = "jdbc:postgresql://" + database.getHost() + ":" + database.getPort() + "/postgres";
				dbos.createPostgresql(driverClass, conUrl, req.getAdminuser(), req.getAdminpass(),
						database.getUserName(), database.getPassword(), database.getDbName());
			} else if ("oracle".equals(database.getDbType())) {
				database.setSid(req.getSidordbname());
				driverClass = "oracle.jdbc.driver.OracleDriver";
				conUrl = "jdbc:oracle:thin:@" + database.getHost() + ":" + database.getPort() + ":" + database.getSid();
				dbos.createOracle(driverClass, conUrl, req.getAdminuser(), req.getAdminpass(), database.getUserName(),
						database.getPassword());
			} else if ("mysql".equals(database.getDbType())) {
				database.setDbName(req.getSidordbname());
				driverClass = "com.mysql.jdbc.Driver";
				conUrl = "jdbc:mysql://" + database.getHost() + ":" + database.getPort() + "/mysql?characterEncoding=UTF-8";
				dbos.createMysql(driverClass, conUrl, req.getAdminuser(), req.getAdminpass(), database.getUserName(),
						database.getPassword(), database.getDbName());
			}
			LOGGER.debug("创建服务成功");
	...
	public DatabaseDetailResponse testByList(DatabaseDetailRequest req) {
		...
		try {
			...
			if ("postgresql".equals(database.getDbType())) {
				driverClass = "org.postgresql.Driver";
				conUrl = "jdbc:postgresql://" + database.getHost() + ":" + database.getPort() + "/"
						+ database.getDbName();
				dbos.testPostgresql(driverClass, conUrl, database.getUserName(),
						CodecUtil.aesDecrypt(database.getPassword(), DBPasswordEncrypt.encryptKey));
			} else if ("oracle".equals(database.getDbType())) {
				driverClass = "oracle.jdbc.driver.OracleDriver";
				conUrl = "jdbc:oracle:thin:@" + database.getHost() + ":" + database.getPort() + ":" + database.getSid();
				dbos.testOracle(driverClass, conUrl, database.getUserName(),
						CodecUtil.aesDecrypt(database.getPassword(), DBPasswordEncrypt.encryptKey));
			} else if ("mysql".equals(database.getDbType())) {
				driverClass = "com.mysql.jdbc.Driver";
				conUrl = "jdbc:mysql://" + database.getHost() + ":" + database.getPort() + "/" + database.getDbName()
						+ "?characterEncoding=UTF-8";
				dbos.testMysql(driverClass, conUrl, database.getUserName(),
						CodecUtil.aesDecrypt(database.getPassword(), DBPasswordEncrypt.encryptKey));
			}
	...
	public DatabaseTestResponse testDatabase(DatabaseSaveRequest req) {
		...
		try {
			...
			if ("postgresql".equals(database.getDbType())) {
				...
				conUrl = "jdbc:mysql://" + database.getHost() + ":" + database.getPort() + "/" + database.getDbName()
						+ "?characterEncoding=UTF-8";
				dbos.testMysql(driverClass, conUrl, database.getUserName(), database.getPassword());
				LOGGER.debug("测试数据库成功");
			}
		} catch (Exception dbtfe) {
