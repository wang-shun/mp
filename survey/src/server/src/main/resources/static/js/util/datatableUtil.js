define(["jquery","preferenceUtil", "dataTables_min", "datatable_lnpagination","datatable_columresize"], 
	function($,preferenceUtil){
		...
		datatableObj = function(tableObj, parmObj ,callback ,completecallback,domObj) {
			...
			//perPageNum = preferenceUtil.customPreferenceLoad("perPageNum");
			
//			$("#jsFfTips").text("").hide();
		    //查询 checkbox依旧选中的bug修复
			var gridtable = $("#grid-table");
			if(tableObj && tableObj.url){
				if (tableObj.tbID != undefined && tableObj.tbID != "") {
					gridtable = $("#" + tableObj.tbID);
				}
				if ($("#" + tableObj.tbID) && $("#" + tableObj.tbID + " thead").find("input:checkbox").length > 0) {
	                $("#" + tableObj.tbID + " thead").find("input:checkbox").removeAttr("checked");
	            }
			    //是否显示title
				...
				refreshDatable(datatable,tableObj);
...
//					if(that.attr("title")==undefined) return;
//					var titles =that.attr("title");
...
//				})
		...
		// refresh datatable
		refreshDatable = function (datatable, tableObj) {
			$(".datatable_refresh_ln").live("click", function() {
                ...
                gridtable.find("input:checkbox").removeAttr("checked");
...
//				if(checkStatus){
//					if(that.$el.find("#grid-table input[type=checkbox][class!=SID-checkAll]").length>0){
//						that.$el.find(".SID-page-tar").text("全部");
//						that.$el.find(".SID-page-tar").prev().val("all");
//						that.$el.find("#grid-table input[type=checkbox]").attr("checked","checked");
//					}
//				}
//			}
//		}
