define([
    ...
    var view = CommunicationBaseView.extend({
        events: {
        ...
        render:function(data,str){
			...
			if(str == 'preview')
			{
			commonDialog.addBtn("templateUse","引用模板",function(){
				self.pushMsg(data.survey.id);
				self.addUseTimes(data.survey.id);
				self.destroy();
				commonDialog.cancel();
			});
			}
        ...
        addUseTimes : function(surveyId)
        {
        	...
        	$.ajax({
				type:"POST",
				url:servicePath+"?"+ropParam+ "&method=mapps.surveyTemplate.useTimes&surveyId="+surveyId,
