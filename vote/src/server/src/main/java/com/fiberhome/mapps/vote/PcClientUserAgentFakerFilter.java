...
@Component
public class PcClientUserAgentFakerFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		HttpServletRequest wrapper = new HttpServletRequestWrapper(req) {
			...
			public String getHeader(String name) {
				String sh = super.getHeader(name);
				if ("User-Agent".endsWith(name) && (sh != null && sh.contains("mplusPcClient"))) {
