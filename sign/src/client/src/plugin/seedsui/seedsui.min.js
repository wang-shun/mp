...
(function(window,document,undefined){
	...
	window.BaiduMap=function(container,params){
		...
		s.addMarkers=function(markerData){
			var markers = [];
			s.map.setCenter(new BMap.Point(markerData[0][0],markerData[0][1]));
			for(var i=0;i<markerData.length;i++){
				//添加label对象
				if(s.params.labelStyle){
					var myLabel=new BMap.Label(markerData[i][3]);
					s.params.labelStyle.backgroundColor="red";//markerData[i][4];
					myLabel.setStyle(s.params.labelStyle);
				}
				//添加描点
				var marker = new BMap.Marker(new BMap.Point(markerData[i][0],markerData[i][1]));
		...
		s.removeMarkers=function(markers){
			for(var i=0;i<markers.length;i++){
				s.map.removeOverlay(markers[i])
			}
		}
		//获得gps位置信息
...
(function(window,document,undefined){
	...
	window.Slider=function(container,params){
        ...
        s.slideTo=function(slideIndex,init){
        	if(slideIndex>=0){
				s.index=slideIndex;
			}
			//索引不能小于0
...
(function() {
	function DateSelect(opt) {
		...
		var tempOACalenderDate = new Date();
		var preD = new Date();
		function _getMonthCalenderDate(calenderType) {
			if(calenderType === "OACalender"){
				if(this.active_date.getDate() > 20){
					preD.setMonth(this.active_date.getMonth(),21);//考勤月的第一天
				}else{
					preD.setMonth(this.active_date.getMonth()-1,21);
				}
				tempMonthCalenderDate = preD;
				var day = tempMonthCalenderDate.getDay();//考勤月第一天星期数
				var first_date_time = tempMonthCalenderDate.getTime() - dayMilliSecound * day;//日历第一天

				return _getMonthCalenderArrayByTimeAndArray(this.temp_month_calender_date_array, first_date_time);
			}else{
				tempMonthCalenderDate.setTime(this.active_date.getTime() - dayMilliSecound * (this.active_date.getDate() - 1));//当月第一天
				var day = tempMonthCalenderDate.getDay();//当月第一天星期数
		...
		function _getMonthCalenderDateTimeByIndex2(index,calenderType) {
			tempMonthCalenderDate.setTime(this.active_date.getTime() - dayMilliSecound * (this.active_date.getDate() - 1));//当月1号
		...
		function _getMonthCalenderDateTimeByIndex(index,calenderType) {
			if(calenderType === "OACalender"){
				tempMonthCalenderDate.setTime(_getOACalenderDate(this.active_date).getTime());//当前考勤月21号
			}else{
				tempMonthCalenderDate.setTime(this.active_date.getTime() - dayMilliSecound * (this.active_date.getDate() - 1));//当月1号
			}
		 ...
		 */
		function _getOACalenderDate(active_date){
			tempOACalenderDate.setTime(active_date.getTime());
			if(active_date.getDate()<=20){//考勤开始时间是上月21号
				tempOACalenderDate.setMonth(active_date.getMonth()-1,21);
			}else{//考勤开始时间是当月21号
				tempOACalenderDate.setMonth(active_date.getMonth(),21);
			}
			return tempOACalenderDate;
		}
		...
		this.getOACalenderDate = _getOACalenderDate;
	}
	...
	function Calender(opt) {
		...
		this.calenderType = opt.calenderType ? opt.calenderType : 'calender';
		...
		function _drawCalenderDate(_this) {
			...
			var month_container = _this.keyElement.month_container,
				...
				tempThisMonthCalenderDate = _this.dateSelectObject.getMonthCalenderDate(_this.calenderType),
				tempSpanElement,
			...
			if(_this.calenderType === "OACalender"){
				var preD = new Date(),lastD = new Date();
				var cur_date = selectedDate.getDate();
				preD.setTime(selectedDate.getTime());
				lastD.setTime(selectedDate.getTime());
				if(cur_date > 20){
					preD.setMonth(selectedDate.getMonth(),21);
					lastD.setMonth(selectedDate.getMonth()+1,20);
					
				}else{
					preD.setMonth(selectedDate.getMonth()-1,21);
					lastD.setMonth(selectedDate.getMonth(),20);
				}
				//考勤月
				for (i = 0; i < 6; ++i) {
					for (j = 0; j < 7; ++j) {
						...
						/*if(tempThisMonthCalenderDate[k].getTime() < preD.getTime() || tempThisMonthCalenderDate[k].getTime() > lastD.getTime()){
							tempSpanElement.className += " not-this-month";
						}*/
						if(tempThisMonthCalenderDate[k].getMonth() !== preD.getMonth() && tempThisMonthCalenderDate[k].getMonth() !== lastD.getMonth()){
							tempSpanElement.className += " not-this-month";
						}else{
							if(tempThisMonthCalenderDate[k].getDate() > 20 && tempThisMonthCalenderDate[k].getMonth() == lastD.getMonth()){
								tempSpanElement.className += " not-this-month";
							}
							if(tempThisMonthCalenderDate[k].getDate() <= 20 && tempThisMonthCalenderDate[k].getMonth() == preD.getMonth()){
								tempSpanElement.className += " not-this-month";
							}
						}
		...
		function _initDateTouchAndAnimation(_this) {
			...
			_this.switchCalenderType = function() {
				// _this.mode = 'month';
				if(_this.calenderType === 'OACalender'){
					_this.calenderType = 'calender';
				}else{
					_this.calenderType = 'OACalender';
				}
				_drawCalenderDate(_this);
			};

			_this.switchToOACalender = function() {
				_this.mode = 'month';
				_this.calenderType = 'OACalender';
				_drawCalenderDate(_this);
			};

			_this.switchToCalender = function() {
				_this.calenderType = 'calender';
				_drawCalenderDate(_this);
			};
			...
			function handleDateClick(e) {
				...
				tempHandleDateClickDate.setTime(_this.dateSelectObject.getMonthCalenderDateTimeByIndex(dateIndex,_this.calenderType));
