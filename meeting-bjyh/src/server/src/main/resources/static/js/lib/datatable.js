...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		...
		function _fnAddColumn( oSettings, nTh )
		{
			...
			//console.log(oSettings.aoColumnsField)
			var iCol = oSettings.aoColumns.length;
			//列方法
			//console.log(oDefaults)
			if(oSettings.oInit.aoColumnsField)
			{
			var oCol = $.extend( {}, DataTable.models.oColumn, oDefaults, {
				...
				"sField":oSettings.oInit.aoColumnsField[iCol].sFieldName
			} );
			}
			else
			{
			...
			////将列对象放入到列数组容器中(oCol(列对象th对象添加到对象中))
		...
		function _fnGetCellData( oSettings, iRow, iCol, sSpecific )
		{
			...
			var oColName=oCol.sField
			//console.log(oCol.sField)
			//console.log(oData)	
		...
		function _fnCreateTr ( oSettings, iRow )
		{
			...
			var oDataData=oData._aData;
			//console.log(oDataData.length)
			var dataArray={};
			for(var data=0;data<oDataData.length;data++)
			{
				dataArray[oDataData[data].split(":")[0]]=oDataData[data].split(":")[1]
			}
			...
			if ( oData.nTr === null )
			{
				...
				for ( var i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					//console.log(oColName)
					nTd = document.createElement( oCol.sCellType );
					
					/* Render if needed - if bUseRendered is true then we already have the rendered
					 * value in the data source - so can just use that
					 */
					 if(oColName==undefined)
					 {
						nTd.innerHTML = (typeof oCol.fnRender === 'function' && (!oCol.bUseRendered || oCol.mDataProp === null)) ?
							_fnRender( oSettings, iRow, i ) :
							_fnGetCellData( oSettings, iRow, i, 'display' ).split(":")[1];
						//console.log(_fnGetCellData( oSettings, iRow, i, 'display' ))
						$(nTd).attr("fieldname",_fnGetCellData( oSettings, iRow, i, 'display' )[0])
						 }
					 else
					 {
						nTd.innerHTML = (typeof oCol.fnRender === 'function' && (!oCol.bUseRendered || oCol.mDataProp === null)) ?
			...
			if ( oSettings.sAjaxSource !== null && !oSettings.oFeatures.bServerSide )
			{
				...
				oSettings.fnServerData.call( oSettings.oInstance, oSettings.sAjaxSource, aoData, function(json) {
					...
					for ( i=0 ; i<aData.length ; i++ )
					{
						...
						{
							retData.push(key+":"+aData[i][key])
							retJsonData.push('"'+key+'"'+':'+'"'+aData[i][key]+'"')
						}
						sortDataAttr.push("{"+retJsonData+"}")
						//console.log(retData)
						_fnAddData( oSettings, retData );
					}
					$("#"+oSettings.sTableId).data("TableStaticData",sortDataAttr)
					//_fnSetStaicData(sortDataAttr)
		...
		function _fnCalculateEnd( oSettings )
		{
			...
			else
			{
				 ...
				 * still to display
				 */
				var ReflashtotalPage= (Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._iDisplayLength))
				$("#"+oSettings.sTableId+"pagination").siblings(".Js_totalPage").text(ReflashtotalPage)
		 ...
		 *  @memberof DataTable#oApi
		 */
		 //oSettings.oApi._fnBindAction( nChoice,  {action: "choice"}, fnClickHandler );
		...
		var _that = this;
		//$(this).data("fixedTableData",_sortDataAttr)
		return this.each(function() {
			...
			// Map the initialisation options onto the settings object(zhiding fangfa lujing )
			...
			_fnMap( oSettings, oInit, "aoColumnsField");
			...
			/* If not given a column array, generate one with nulls 初始化列 */
			...
			for ( i=0, iLen=aoColumnsInit.length ; i<iLen ; i++ )
			{
				...
				if(oSettings.oInit.aoColumnsField)
				{
				$(anThs[i]).attr("fieldname",oSettings.oInit.aoColumnsField[i].sFieldName)
				}
				_fnAddColumn( oSettings, anThs ? anThs[i] : null );
			}
	...
	$.extend( DataTable.ext.oStdClasses, {
		...
		"sPagePreStaticDisabled":"pre_button_disabled",
		"sPageFirstStaticDisabled":"first_button_disabled",
		"sPageLastStaticDisabled":"last_button_disabled",
		"sPageEndStaticDisabled":"end_button_disabled",		
		"sPageFirst": "first",
	...
	$.extend( DataTable.ext.oPagination, {
		...
		"full_numbers": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				var totalPage= (Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._iDisplayLength))
				$(nPaging).append(
					'<a  tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageFirst+'">'+oLang.sFirst+'</a>'+
					'<a  tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPagePrevious+'">'+oLang.sPrevious+'</a>'+
					'<span class="Js_linkPage" id="'+oSettings.sTableId+'_linkpage"></span>'+'<span class="dropSplit"></span>'+'<span class="">第<input type="text" id="'+oSettings.sTableId+'pagination" class="Js_pagination pagination_jump" value="" />共<em class="Js_totalPage" id="'+oSettings.sTableId+'"_totalPage>'+totalPage+'</em>页</span>'+'<span class="dropSplit"></span>'+
					'<a tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageNext+'">'+oLang.sNext+'</a>'+
					'<a tabindex="'+oSettings.iTabIndex+'" class="'+oClasses.sPageButton+" "+oClasses.sPageLast+'">'+oLang.sLast+'</a><span class="dropSplit"></span><span id="'+oSettings.sTableId+'_reflash" class="reflash"></span>'
				);
				$("#"+oSettings.sTableId+"pagination").live("keyup",function(){
						var _this=$(this)
						...
						{
							valNum=parseInt(val)
							var linkpagebox=$("#"+oSettings.sTableId+"_linkpage")
							var linkPage=$("#"+oSettings.sTableId+"_linkpage > a")
							var totalPage=$("#"+oSettings.sTableId+"_totalPage");
								if(valNum>=parseInt((totalPage.text())))
								{
									$("#"+oSettings.sTableId+"_last").click();
									//$(".paging_full_numbers .last").click();
								}
								if(valNum<=1)
								{
									$("#"+oSettings.sTableId+"_first").click();
									//$(".paging_full_numbers .first").click();
								}
								else
								{
									//在正常范围内点击
									var curpage=parseInt(linkpagebox.find("a.paginate_active").text())
									if(curpage>valNum)
									{
										//当前页面比输入的页数大（后退操作）
										for(var curIndex=0;curIndex<linkPage.length;curIndex++)
										{
											if(parseInt(linkPage.eq(curIndex).text())==valNum)
											{
												linkPage.eq(curIndex).click();
												}
											}
										}
									else{
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				for ( i=0, iLen=an.length ; i<iLen ; i++ )
				{
					...
					$(anStatic).removeClass( oClasses.sPageButton+" "+oClasses.sPageButtonActive+" "+oClasses.sPageFirstStaticDisabled+" "+oClasses.sPagePreStaticDisabled+" "+oClasses.sPageLastStaticDisabled+" "+oClasses.sPageEndStaticDisabled);
					$(anStatic[0]).addClass(
						(iCurrentPage==1) ?
							oClasses.sPageFirstStaticDisabled :
							oClasses.sPageButton
					)
					$(anStatic[1]).addClass(
						(iCurrentPage==1) ?
							oClasses.sPagePreStaticDisabled :
							oClasses.sPageButton
					)
					$(anStatic[2]).addClass(
						(iPages===0 || iCurrentPage===iPages || oSettings._iDisplayLength===-1) ?
							oClasses.sPageLastStaticDisabled :
							oClasses.sPageButton
					)
					$(anStatic[3]).addClass(
						(iPages===0 || iCurrentPage===iPages || oSettings._iDisplayLength===-1) ?
							oClasses.sPageEndStaticDisabled :
							oClasses.sPageButton
					)
				}
			}
