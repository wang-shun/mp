define(['jquery', 'views/communication-base-view', ''], function($,CommunicationBaseView, TemplateMenu) {
	var MenuSnippetView = CommunicationBaseView.extend({
		...
		setUpContent: function() {
			...
			$.ajax({
				type:"POST",
				url:servicePath+"?"+ropParam+ "&method=mapps.meeting.meeting.role",
				success:function(ajax){
					if(ajax.code == "1"){
						if (ajax.adminFlag) {
							...
							var html = '<li class="nav-category-list"><a class="SID-menu" data-name="预定管理"><span class="line"></span><span class="fhicon-meeting-clock"></span>预定管理</a></li>';	
							self.$el.append(html);
							self.defauleMenu();
							$(".SID-welcome").hide();
						}
