...
@ServiceMethodBean
public class MRThirdPartAccessService {
	protected final Logger LOGGER = LoggerFactory.getLogger(getClass());
	@Autowired
	MplusAccessService accessService;

	@ServiceMethod(method = "mapps.mr.thirdpart.mobileark.getdepartments", group = "thirdpart", groupTitle = "文件服务", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
	public GetDepartmentsResponse getDepartments(GetDepartmentsRequest request) throws Exception {
		...
		try {
			LOGGER.info("获取部门接口(mapps.mr.thirdpart.mobileark.getdepartments)入口");
	...
	@ServiceMethod(method = "mapps.mr.thirdpart.mobileark.getusers", group = "thirdpart", groupTitle = "文件服务", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
	public GetUsersResponse getUsers(GetUsersRequest request) throws Exception {
		GetUsersResponse response = new GetUsersResponse();
		try {
			LOGGER.info("获取成员接口(mapps.mr.thirdpart.mobileark.getusers)入口,请求参数==" + LogUtil.getObjectInfo(request));
			...
			// 如果有orgdepid,则通过orgdepid进行搜索,通过权限控制搜索的范围------------最新需求,不需要添加权限控制
			if (SessionContext.getOrgDeptId() == null)
            {
                response.setUserInfos(accessService.getUsersByName(SessionContext.getOrgId(), userName, 20));
            }
	...
	// @Async
	// public void pushSysMsg(String ecid, String userId, String title, String
	// content)
	// {
	// LOGGER.debug("系统消息推送,ecid={},userId={},title={},content={}", ecid,
	...
	public boolean pushReceiveEvent(String ecid, String userIds, String title, String content, String reservedId,
		...
		try {
			...
			if (StringUtil.isNotEmpty(reservedId)) {
				scheme = "#ID-PageCheckDetail/" + reservedId;
			}
			PushTask.addJobForReceiveEvent(ecid, userIds, title, content, scheme, null);
			// accessService.pushReceiveEvent(ecid, userIds, title, content,
			// scheme);
			// accessService.pushReceiveEvent(ecid, userIds, title, content,
			// scheme, MplusAccessService.APPTYPE_HTML5,
			// param);
		} catch (Exception e) {
