...
{
    @Autowired
    MRThirdPartAccessService thirdPartAccessService;
    ...
    public QueryRoomDetailResponse getMrRoomDetail(QueryRoomDetailRequest req)
    {
        ...
        try
        {
            ...
            // bjyh 新增预约时间查询 start
            if (StringUtils.isNotEmpty(req.getSelectMonth()))
            {
                map.put("selectMonth", req.getSelectMonth());

                List<ReservedInfo> riList = mrReservedMapper.getReservedList(map);
                List<String> loginIdList = new ArrayList<String>();
                for (ReservedInfo riInfo : riList)
                {
                    loginIdList.add(riInfo.getLoginId());
                }
                if (!loginIdList.contains(userId))
                {
                    loginIdList.add(userId);
                }
                ...
                for (ReservedInfo riInfo : riList)
                {
                    ...
                    {
                        if (mu.getLoginId().equals(riInfo.getLoginId()))
                        {
                            riInfo.setUserName(mu.getUserName());
                            riInfo.setTel(mu.getPhoneNum());
                        }
                    }
                }
                res.setReservedList(riList);
            }
            // bjyh end
