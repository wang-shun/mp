...
import com.fiberhome.mapps.meeting.dao.MrFavoriteMapper;
import com.fiberhome.mapps.meeting.dao.MrPrivilegeMapper;
import com.fiberhome.mapps.meeting.dao.MrReservedMapper;
import com.fiberhome.mapps.meeting.dao.MrRoomMapper;
import com.fiberhome.mapps.meeting.entity.GetMrRoom;
import com.fiberhome.mapps.meeting.entity.MrFavorite;
...
import com.fiberhome.mapps.meeting.entity.RoomLogContent;
...
public class MrRoomService
{
    ...
    String                        webRoot;
    @Value("${meetingroom.privilage.whiteList}")
    boolean                       whiteList;
     ...
     * @return
     */
    @ServiceMethod(method = "mapps.meetingroom.room.queryfromWebPage", group = "room", groupTitle = "API", version = "1.0", ignoreSign = IgnoreSignType.YES, needInSession = NeedInSessionType.YES)
    public QueryRoomResponse getMrRoomForPageFromWeb(QueryRoomRequest req)
    {
    	return getMrRoomForPageBase(req, SessionContext.isAdmin());
    }
    ...
    {
        return getMrRoomForPageBase(req, false);
    }
    
    
    private QueryRoomResponse getMrRoomForPageBase(QueryRoomRequest req, boolean isAdmin)
    {
        ...
        try
        {
            ...
            Map<String, Object> map = initQuery(req, isAdmin);
            ...
            else
            {
                for (GetMrRoom info : list)
                {
                    if (StringUtil.isNotEmpty(info.getLayout()))
                        info.setLayout(webRoot + info.getLayout());
                }
            }
    ...
    public Map<String, Object> initQuery(QueryRoomRequest req, boolean isAdmin) throws Exception
    {
        ...
        // else
        // {
        map.put("order", "collection desc,capacity desc,create_time desc");
        // }
        map.put("time", time);
    ...
    public void setDetailResponse(QueryRoomDetailResponse res, List<GetMrRoom> list, List<ReservedDate> dList)
    {
        ...
        {
            res.setImagePath(webRoot + info.getLayout());
        }
        res.setLayout(StringUtils.isEmpty(info.getLayout()) ? "" : info.getLayout());
        res.setRemarks(StringUtils.isEmpty(info.getRemarks()) ? "" : info.getRemarks());
