define([
    ...
    var view = Backbone.View.extend({
        ...
        render:function(){
            ...
            this.listContainer=this.$el.find("#ID-Search-List");
            ...
            //筛选框
            this.dialog=new Dialog("#ID-Search-DialogFilter",
                {
        ...
        loadData:function(){
        	this.$el.find(".SID-Search-State.active").click();
        },
        ...
        refresh:function(){
            this.dragFlag=0;
            console.log("搜索页searchView：刷新");
            this.$el.find("article").scrollTop(0);
        },
        destroy:function(){
            console.log("搜索页searchView：移除");
            this.txtSearch.value="";
            this.txtSearch.nextElementSibling.style.display="none";
            this.listContainer[0].innerHTML=app.nodataHTML;
            this.$el.find(".SID-Search-State")[0].click();
            this.drag.bottomNoData();
        },
        ...
        reloadPage:function(nextPageFlag,topFourFlag){
            ...
            var order = self.$el.find(".SID-Search-State.active").attr("data-status");
                ...
                success:function(data){
                    if(data.code==1){//请求错误
	                    ...
	                    if (!nextPageFlag) {//头部刷新
	                    	if (topFourFlag == 1) {
	                    		self.$el.find("#ID-Search-List").append(listHTML);
	                        	//self.listContainer[0].innerHTML+=listHTML;
	                        } else {
	                        	self.$el.find("#ID-Search-List").html(listHTML);
	                        	//self.listContainer[0].innerHTML=listHTML;
	                        }
	                        setTimeout(function(){
	                            if (self.endflag) {
        ...
        updateStyle:function(){
        	var self = this;
        	var status = self.$el.find(".SID-Search-State.active").attr("data-status");
        ...
        events: {
            ...
            'click #ID-Search-BtnFilter' : '_onClickBtnFilter',
            'click .SID-Search-State':'_onClickState',
        ...
        _onClickState:function(e){
            ...
            this.$el.find(".SID-Search-State.active").removeClass("active");
            obj.addClass("active");
            this.$el.find("#ID-Search-BtnFilter>.app-stateFilterBar-font>span").text(obj.find("div").text());
            if(!this.dialog.isHid){
            	this.$el.find("#ID-Search-BtnFilter").click();
            }
